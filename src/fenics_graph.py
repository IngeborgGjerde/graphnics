...
class FenicsGraph(nx.Graph):
    ...
    def make_mesh(self): 
        ...
        # Mark vertices with vertex ix from graph
        bifurcation_ixs = []
        boundary_ixs = []
        for v in self.nodes():
            connected_eges = self.edges(v)
            if len(connected_eges)==3: bifurcation_ixs.append(v)
            elif len(connected_eges)==1: boundary_ixs.append(v)            
    ...
    def assign_tangents(self):
        ...
        for u,v in self.edges():
            tangent = np.asarray(self.nodes[u]['pos'])-np.asarray(self.nodes[v]['pos'])
        ...
        tangent_i = interpolate(tangent, VectorFunctionSpace(self.global_mesh, 'DG', 0, 3) )
